...
class Model(ModelTemplate):
    ...
    def __init__(self, model_cfg: dict[str, Any]):
        if model_cfg.get("tactile_input", "zero") != "zero":
            raise ValueError("This standard benchmark adapter uses physical-zero observed tactile")
        ...
        import sys
        runtime_root = Path(os.environ["ME_DEX_ROOT"]).expanduser().resolve() / "runtime"
        sys.path.insert(0, str(runtime_root))
        from policy import MEDexPolicy

        resolved_cfg = dict(model_cfg)
        if os.environ.get("ME_DEX_CHECKPOINT_PATH"):
            resolved_cfg["checkpoint_path"] = os.environ["ME_DEX_CHECKPOINT_PATH"]
        checkpoint = resolve_checkpoint_root(resolved_cfg, BENCH_ROOT / "checkpoints")
        ...
        self.policy = MEDexPolicy(
            ...
            input_color_order=str(os.environ.get("ME_DEX_INPUT_COLOR_ORDER", model_cfg.get("input_color_order", "rgb"))),
        )
